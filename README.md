...
1. Generate tim file with injected Omega_gw, which is a power law (with given amplitude and spectral index nt), with createGWB:
   ```
...
2. Generate tim file with injected Omega_gw, which is a power law (with given amplitude and spectral index nt), with add rednoise:
   ```
   python inject_omega_red_noise.py --datadir epta_sim/ --comp 30 --iter_num 1e5 --iter_real 1 --datadir_out output_7/ --nt 3.4 --amp 5e-8
   ```   
...
output_6/ --nt 3.4 --amp 5e-8 (with spectral leackage)

output_7/ --nt 3.4 --amp 5e-8 (without spectral leackage)
